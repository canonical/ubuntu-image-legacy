"""Internal, unstable API compatibility module.
...
class Origin:
    ...
    This class supports "pinpointing" something in a block of text.  The
    block is described by the source attribute.  The actual range is
    described by line_start (inclusive) and line_end (exclusive).
    ...
    def __str__(self):
        ...
        if mode is OriginMode.whole_file:
            ...
            raise AssertionError('Bad mode: {}'.format(mode))

    def relative_to(self, base_dir):
        """Create a new relative Origin.

        The new Origin has a source relative to the specified base directory.
    ...
    def with_offset(self, offset):
        """Create a new Origin offset by a number of lines.

        The new Origin adds an offset of a specific number of lines.
        ...
        if mode is OriginMode.whole_file:
            return self
        elif mode in (OriginMode.single_line, OriginMode.line_range):
            return Origin(
...
class FileTextSource:
    ...
    def relative_to(self, base_dir):
        """Return a FileTextSource with a new path.

        This computes a FileTextSource with the filename being a
        relative path from the specified base directory.
...
class RFC822Record:
    ...
    def __init__(self, data, origin=None, raw_data=None,
        ...
        :param field_offset_map:
            An optional dictionary with offsets (in line numbers) of
            each field.
        """
        self._data = data
        self._raw_data = (data if raw_data is None else raw_data)
        self._origin = origin
...
def gen_rfc822_records(stream, data_cls=dict, source=None):
    ...
    if isinstance(stream, str):
        stream = iter(stream.splitlines(keepends=True))
